            ...
            <div class="w-full flex flex-col gap-5">
                ...
                <div class="w-full bg-white/80 backdrop-blur-sm rounded-3xl border border-[#ecdfff] shadow-sm px-5 sm:px-7 py-5">
                    <ol class="flex items-center gap-2">
                        ...
                        @endphp
                        @foreach ($steps as $si => $sname)
                            @php $sn = $si + 1; @endphp
                            <li class="flex items-center gap-2 step-item {{ $sn === 1 ? 'step-active' : 'step-inactive' }}" data-for="{{ $sn }}">
                                <span class="step-num w-8 h-8 sm:w-9 sm:h-9 inline-flex items-center justify-center rounded-full font-bold text-sm border-2 shrink-0 transition-all">{{ $sn }}</span>
                                <span class="text-[10px] sm:text-sm font-bold text-[#6d2857] whitespace-nowrap step-lbl">{{ $sname }}</span>
                            </li>
                            @if (!$loop->last)
                                <li class="step-line flex-1 h-0.5 bg-[#d9c7ee] rounded-full mx-1 transition-all" data-line="{{ $sn }}"></li>
                            @endif
                        @endforeach
                    </ol>
                </div>

                {{-- -------- PANEL 1: Informasi Pengiriman -------- --}}
                ...
                <div class="w-full bg-white/85 backdrop-blur-sm rounded-3xl border border-[#ecdfff] shadow-[0_14px_34px_rgba(109,40,87,0.08)] p-5 sm:p-6">
                            ...
                            @foreach ($cart as $item)
                                <div class="flex items-center gap-3">
                                    <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-gradient-to-b from-[#8c52a5] to-[#6e3787] p-1.5 shrink-0 overflow-hidden">
                                        <img src="{{ asset('images/landing/' . $item['img']) }}" alt="{{ $item['name'] }}"
                                             class="w-full h-full object-contain"/>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-bold text-[#3d1f48] truncate">{{ $item['name'] }}</p>
                                        <p class="text-xs text-[#8b5cf6]">{{ $item['tag'] }}</p>
                                        <p class="text-sm font-bold text-[#6d2857]">Rp {{ number_format($item['price'], 0, ',', '.') }}</p>
                                    </div>
                                    <div class="flex items-center border border-[#d9c7ee] rounded-full overflow-hidden bg-white shrink-0">
                                        <span class="w-7 h-7 inline-flex items-center justify-center text-xs font-bold text-[#6d2857] border-r border-[#d9c7ee]">{{ $item['qty'] }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="space-y-2 border-t border-[#ecdfff] pt-4 mb-4">
                        <div class="flex justify-between text-sm text-gray-600">
                            <span>Subtotal ({{ array_sum(array_column($cart, 'qty')) }} items)</span>
                            <span class="font-bold text-[#6d2857]">Rp {{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm text-gray-600">
                            <span>Ongkos Kirim</span>
                            <span id="ongkir-display" class="font-bold text-[#6d2857]">Rp {{ number_format($ongkir, 0, ',', '.') }}</span>
                        </div>
                    </div>
